...
describe("transaction", () => {
  ...
  test("it should return user infor", async () => {
     ...
     */
     function getKey(secret: string): Promise<CryptoKey> {
      return crypto.subtle.importKey(
        "raw",
        ENCODER.encode(secret.padEnd(32, "0")).slice(0, 32), // ensure 256-bit key
        { name: "AES-GCM" },
        false,
        ["encrypt", "decrypt"]
      );
     ...
     */
    async function encryptWithKey(secret: string, data: string): Promise<EncryptedPayload> {
    ...
    /**
     * Decrypt payload with secret
     */
    async function decryptWithKey(secret: string, payload: EncryptedPayload): Promise<string> {
      const key = await getKey(secret);
    ...
    (async () => {
      const secret = "MY_PARTNER_API_KEY";
      const message = "sensitive-data";

      const encrypted = await encryptWithKey(secret, message);
      console.log("Encrypted:", encrypted);

      const decrypted = await decryptWithKey(secret, encrypted);
